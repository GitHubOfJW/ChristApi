...
class Solar {
  ...
  constructor(date){
    ...
    // 阳历此年此月的第一天
   const firstDayDate = new Date(year,month,1,0,0,0,0);
    // 获取当月的天数
    this.days = Solar.getDaysOfSolarMonth(year,month);
    // 获取公元1日星期几
   this.firstDayWeek = firstDayDate.getDay();
   this.firstDayDate = moment(firstDayDate).toString()

   // 今天星期几
   this.week = date.getDay()

    // 立春
    this.springOfBegin = Solar.getYearTerm(year,2)
    ...
    // 获取当月中的节气从几日开始
    this.monthFirstNode = Solar.getYearTerm(year,month*2);
